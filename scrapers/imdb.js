...
async function clickLoadMore(times) {
    for (let i = 0; i < times; i++) {
        ...
        const btn = document.querySelector(
            "button.ipc-btn--single-padding:nth-child(1)"
        );
...
const movies = [
    ...
    let idMatch = (film.querySelector(".ipc-lockup-overlay")?.href ?? "").match(
        /title\/(tt\d+?)\//
    );
    let year = (film.querySelector(".sc-f30335b4-7")?.innerHTML ?? "")
        .trim()
        .slice(0, 4);
